...
function GallerySlider() {
  ...
  return (
    <div className="relative w-full overflow-hidden group mb-12 select-none px-6">
      <div className="max-w-7xl mx-auto overflow-hidden rounded-xl bg-[#F9F7F4] p-1 border border-[#C8A96E]/15">
        <motion.div
          animate={{
        ...
        >
          {displayPhotos.map((photo, i) => (
            <div
              key={i}
              className="min-w-full md:min-w-[calc(50%-4px)] h-full relative rounded overflow-hidden"
            >
      ...
      <div className="absolute inset-x-4 md:inset-x-8 top-1/2 -translate-y-1/2 flex justify-between z-40 pointer-events-none">
        <button onClick={prev} className="pointer-events-auto p-2 md:p-3 text-[#1A1A1A] opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all bg-white/80 hover:bg-white border border-[#C8A96E]/20">
          <ChevronLeft className="w-4 h-4 md:w-5 md:h-5" />
        </button>
        <button onClick={next} className="pointer-events-auto p-2 md:p-3 text-[#1A1A1A] opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all bg-white/80 hover:bg-white border border-[#C8A96E]/20">
      ...
      <div className="flex justify-center gap-2 mt-8">
        {Array.from({ length: displayPhotos.length - 1 }).map((_, i) => (
          <button
            key={i}
            onClick={() => setIndex(i)}
            className={cn("h-px rounded-none transition-all duration-500", i === index ? "bg-[#1A1A1A] w-10" : "bg-[#C8A96E]/40 w-5")}
          />
        ))}
      </div>
    </div>
    ...
    <section className="py-24 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="max-w-2xl mx-auto px-6 text-center mb-16">
          ...
          <h2 className="text-4xl md:text-6xl font-serif text-[#1A1A1A] font-light mb-4">Galeri Kenangan</h2>
          <div className="w-12 h-px bg-[#C8A96E]/40 mx-auto mt-6" />
        </div>
          ...
          {photos.map((photo, index) => (
            ...
            >
              <Image
                ...
                className="object-cover transition-transform duration-700 group-hover:scale-105 object-cover"
              />
            </motion.div>
          ))}
        </div>
...
export function RSVPSection({ guestName }: { guestName?: string }) {
  ...
  return (
    <section className="py-20 px-8 bg-[#F9F7F4]">
      <div className="max-w-xl mx-auto">
        <div className="text-center mb-12">
          <span className="text-[#C8A96E] text-[9px] uppercase tracking-[0.5em] font-light block mb-4">RSVP</span>
          <h2 className="text-3xl md:text-5xl font-serif text-[#1A1A1A] font-light mb-2">Konfirmasi Kehadiran</h2>
          <div className="w-12 h-px bg-[#C8A96E]/40 mx-auto mt-4" />
        </div>
          ...
          <form className="space-y-7">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="space-y-2">
                <label className="text-[9px] uppercase tracking-[0.3em] text-[#C8A96E] font-light block">Nama Lengkap</label>
                <input
                  type="text"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  className="w-full bg-transparent border-b border-[#C8A96E]/30 pb-2 text-sm text-[#1A1A1A] focus:border-[#C8A96E] outline-none transition-all placeholder:text-stone-300 font-light"
                  placeholder="Masukkan nama Anda"
                />
              </div>
              <div className="space-y-2">
                <label className="text-[9px] uppercase tracking-[0.3em] text-[#C8A96E] font-light block">Jumlah Tamu</label>
                <CustomSelect
                  ...
                  theme={{
                    ...
                    optionHover: "hover:bg-[#F9F7F4]",
                    optionText: "text-[#1A1A1A]",
                    checkColor: "text-[#C8A96E]",
                    chevronColor: "text-[#C8A96E]",
                    triggerClass: "bg-transparent border-b border-[#C8A96E]/30 pb-2 font-light outline-none",
                    panelClass: "bg-white border border-[#C8A96E]/20 rounded-lg shadow-md",
                  }}
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-[9px] uppercase tracking-[0.3em] text-[#C8A96E] font-light block">Status Kehadiran</label>
                ...
                {["Hadir", "Tidak Hadir"].map((s) => (
                  <button
                    ...
                    className={cn(
                      ...
                      status === s
                        ? "bg-[#1A1A1A] text-[#F9F7F4] border-[#1A1A1A]"
                        : "bg-transparent text-stone-400 border-[#C8A96E]/30 hover:border-[#C8A96E]/60"
                    )}
                  >
                    {s}
                  </button>
            ...
            <div className="space-y-2">
              <label className="text-[9px] uppercase tracking-[0.3em] text-[#C8A96E] font-light block">Pesan atau Ucapan</label>
              <textarea
                className="w-full bg-transparent border-b border-[#C8A96E]/30 pb-2 text-sm text-[#1A1A1A] focus:border-[#C8A96E] outline-none h-28 transition-all resize-none placeholder:text-stone-300 font-light"
                placeholder="Tulis ucapan selamat..."
              />
            </div>

            <button className="w-full border border-[#1A1A1A] text-[#1A1A1A] py-4 font-light uppercase tracking-[0.3em] text-[10px] hover:bg-[#1A1A1A] hover:text-[#F9F7F4] transition-all">
              Kirim Konfirmasi
            </button>
          </form>
        </motion.div>
        ...
        <div className="mt-24 text-center pb-16">
          <span className="text-[#1A1A1A] text-5xl md:text-7xl font-accent block mb-5">Thank You</span>
          <div className="flex items-center justify-center gap-3 mb-4">
            <div className="h-px w-12 bg-[#C8A96E]/40" />
            <span className="text-[#C8A96E]/60 text-xs">◇</span>
            <div className="h-px w-12 bg-[#C8A96E]/40" />
          </div>
